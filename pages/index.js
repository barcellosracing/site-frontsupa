...
export default function Home(){
  ...
  async function load(){
    const [{data:clients},{data:products},{data:services},{data:quotes},{data:investments}] = await Promise.all([
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="card">
          <div className="text-sm">Orçamentos</div>
      ...
      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="card">
          <div className="text-sm">Investimentos (Despesas)</div>
          <div className="text-2xl font-bold">R$ {investTotal.toFixed(2)}</div>
          <Link href="/investments"><a className="text-sm text-accent">Gerenciar investimentos →</a></Link>
        </div>
      </div>
    </div>
  )
