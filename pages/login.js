...
export default function Login(){
  ...
  function handle(e){
    ...
    if(code.trim() === 'br.admin'){
      ...
      alert('Acesso de admin ativado por 1 hora.')
      router.push('/')
    } else {
      alert('Código inválido. Modo visualização ativado.')
  ...
  return (
    <div className="max-w-md mx-auto card">
      ...
      <form onSubmit={handle}>
        <label className="block text-sm mb-2">Código de acesso</label>
        <input className="w-full p-2 border rounded mb-4" value={code} onChange={e=>setCode(e.target.value)} />
        <button className="w-full p-2 rounded btn">Entrar</button>
      </form>
    </div>
  )
}
