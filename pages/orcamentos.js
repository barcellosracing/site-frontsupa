...
export default function Orcamentos(){
  ...
  async function saveBudget(e){ e.preventDefault(); if(!isAdmin()){ alert('Apenas admin pode criar orçamentos'); return } if(!clientId){ alert('Selecione cliente'); return } if(items.length===0){ alert('Adicione pelo menos 1 item'); return } const total = items.reduce((s,it)=> s + (it.price * it.qty), 0); const created_at = new Date().toISOString(); const { data:budget, error } = await supabase.from('budgets').insert([{ client_id: clientId, total, status: 'pending', created_at }]).select().single(); if(error){ alert(error.message); return } const bi = items.map(it=>({ budget_id: budget.id, item_type: it.type==='product' ? 'product' : 'service', item_id: it.id, quantity: it.qty, price: it.price, created_at })); const { error: err2 } = await supabase.from('budget_items').insert(bi); if(err2){ alert(err2.message); return } setItems([]); setClientId(''); fetchAll(); alert('Orçamento salvo') }
